...
type Formats = 'date-time' | 'time' | 'date' | 'duration' | 'email' | 'idn-email' | 'hostname' | 'idn-hostname' | 'ipv4' | 'ipv6' | 'uuid' | 'uri' | 'uri-reference' | 'iri' | 'iri-reference' | 'uri-template' | 'json-pointer' | 'relative-json-pointer' | 'regex';
type StringSchema = {
    type: 'string';
    enum?: string[];
    minLength?: number;
...
type JSONSchemaOfType<T extends BaseType> = T extends 'string' ? StringSchema : T extends 'number' ? NumberSchema : T extends 'boolean' ? BooleanSchema : T extends 'object' ? ObjectSchema : T extends 'array' ? ArraySchema : T extends 'discriminatedUnion' ? ObjectSchema : never;
type BaseType = 'number' | 'string' | 'boolean' | 'object' | 'array' | 'discriminatedUnion';
declare const containerTypes: readonly ["object", "array", "discriminatedUnion"];
...
type ZuiReactDiscriminatedUnionComponentProps<Type extends ContainerType, ID extends keyof UI[Type], UI extends UIComponentDefinitions = DefaultComponentDefinitions> = ZuiReactComponentBaseProps<Type, ID, UI> & {
...
type ZuiReactComponentProps<Type extends BaseType, ID extends keyof UI[Type], UI extends UIComponentDefinitions = DefaultComponentDefinitions> = Type extends 'object' ? ZuiReactObjectComponentProps<Type, ID, UI> : Type extends 'array' ? ZuiReactArrayComponentProps<Type, ID, UI> : Type extends 'discriminatedUnion' ? ZuiReactDiscriminatedUnionComponentProps<Type, ID, UI> : ZuiReactControlComponentProps<Type, ID, UI>;
...
type MergeUIComponentDefinitions<T extends UIComponentDefinitions, U extends UIComponentDefinitions> = {
    [Type in BaseType]: {
...
declare class UntitledDeclarationError extends Error {
...
declare function toTypescript(schema: z.Schema, options?: TypescriptGenerationOptions): string;
...
declare abstract class ZodType<Output = any, Def extends ZodTypeDef = ZodTypeDef, Input = Output> {
     ...
     * @default false
     */
    hidden<T extends any = this['_output']>(hidden?: boolean | ((shape: T | null) => util.DeepPartialBoolean<T> | boolean)): this;
    /**
     * Whether the field is disabled
     * @default false
     */
    disabled<T extends any = this['_output']>(disabled?: boolean | ((shape: T | null) => util.DeepPartialBoolean<T> | boolean)): this;
    /**
    ...
    toTypescriptAsync(opts?: Omit<TypescriptGenerationOptions, 'formatter'> & {
...
declare const templateLiteralType: (params?: RawCreateParams & {
    coerce?: true;
}) => ZodTemplateLiteral;
...
declare const coerce: {
    ...
    templateLiteral: (typeof ZodTemplateLiteral)["create"];
};
...
type BoundaryFallbackComponent = FC<{
    error: Error;
    schema: JSONSchema;
}>;
