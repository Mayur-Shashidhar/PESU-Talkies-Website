...
// src/transforms/zui-to-typescript-next/utils.ts
...
// src/transforms/zui-to-typescript-next/index.ts
var Primitives = [
  "string",
  "number",
  "boolean",
...
var _UntitledDeclarationError = class _UntitledDeclarationError extends Error {
...
function toTypescript(schema, options) {
  ...
  (_a = options.declaration) != null ? _a : options.declaration = false;
  let wrappedSchema = schema;
  if (options == null ? void 0 : options.declaration) {
    if (schema instanceof z_default.Schema) {
  ...
  let dts = sUnwrapZod(wrappedSchema, __spreadValues({}, options));
...
function sUnwrapZod(schema, config) {
  ...
  if (schema instanceof Declaration) {
    ...
    const typings = sUnwrapZod(withoutDesc, __spreadProps(__spreadValues({}, newConfig), { declaration: true }));
    if (schema.schema instanceof z_default.ZodFunction) {
  ...
  switch (def.typeName) {
    ...
    case z_default.ZodFirstPartyTypeKind.ZodObject:
      const props = Object.entries(schema.shape).map(([key, value]) => {
        if (value instanceof z_default.Schema) {
...
${options.join(" | ")}`;
    case z_default.ZodFirstPartyTypeKind.ZodDiscriminatedUnion:
      const opts = schema.options.map((option) => {
        return sUnwrapZod(option, newConfig);
      });
...
${opts.join(" | ")}`;
    ...
    case z_default.ZodFirstPartyTypeKind.ZodFunction:
      ...
      if (config == null ? void 0 : config.declaration) {
        return `${getMultilineComment(def.description)}
(${input4}): ${output4}`;
      }
      return `${getMultilineComment(def.description)}
(${input4}) => ${output4}`;
    ...
    case z_default.ZodFirstPartyTypeKind.ZodLiteral:
      return `${getMultilineComment(def.description)}
${escapeString(schema.value)}`.trim();
    ...
    case z_default.ZodFirstPartyTypeKind.ZodOptional:
      if ((config == null ? void 0 : config.declaration) || (config == null ? void 0 : config.parent) instanceof z_default.ZodRecord || (config == null ? void 0 : config.parent) instanceof z_default.ZodObject) {
        return `${sUnwrapZod(def.innerType, newConfig)} | undefined`;
      }
      if ((config == null ? void 0 : config.parent) instanceof z_default.ZodDefault || (config == null ? void 0 : config.parent) instanceof z_default.ZodNullable || (config == null ? void 0 : config.parent) instanceof z_default.ZodOptional) {
        return `${sUnwrapZod(def.innerType, newConfig)} | undefined`;
      }
      return `${sUnwrapZod(def.innerType, newConfig)}?`;
    case z_default.ZodFirstPartyTypeKind.ZodNullable:
      return `${sUnwrapZod(schema.unwrap(), newConfig)} | null`;
    case z_default.ZodFirstPartyTypeKind.ZodDefault:
      return sUnwrapZod(def.innerType, newConfig);
    case z_default.ZodFirstPartyTypeKind.ZodCatch:
      return sUnwrapZod(schema.removeCatch(), newConfig);
    case z_default.ZodFirstPartyTypeKind.ZodPromise:
      return `Promise<${sUnwrapZod(schema.unwrap(), newConfig)}>`;
    ...
    case z_default.ZodFirstPartyTypeKind.ZodTemplateLiteral:
      const inner = def.parts.map((p) => {
        ...
        return "${" + sUnwrapZod(p, __spreadProps(__spreadValues({}, newConfig), { declaration: false })) + "}";
      }).join("");
      return `\`${inner}\``;
    default:
...
var _ZodUnion = class _ZodUnion extends ZodType {
  ...
  getReferences() {
    return this._def.options.reduce((acc, option) => {
...
var coerce = {
  ...
  templateLiteral: /* @__PURE__ */ __name((arg) => ZodTemplateLiteral.create(__spreadProps(__spreadValues({}, arg), {
    coerce: true
  })), "templateLiteral")
};
var NEVER = INVALID;
